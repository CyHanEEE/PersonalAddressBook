...
public class OperatePersonFrame extends JFrame {
    ...
    private void initButton() {
        ...
        operateButton.addActionListener(e -> {
            if(businessLogic.validateContactInfo(IName.getText(), IAddress.getText(), IPhone.getText()) == Input.Input_Empty){
                JOptionPane.showMessageDialog(this, "请输入完整的联系人信息", "提示", JOptionPane.WARNING_MESSAGE);
            }
            else if(businessLogic.validateContactInfo(IName.getText(), IAddress.getText(), IPhone.getText()) == Input.Input_Phone_Not_Number){
                JOptionPane.showMessageDialog(this, "电话号码只能为数字", "提示", JOptionPane.WARNING_MESSAGE);
            }
             else {
                try {
                    boolean res = false;  // 电话号码是否存在
                    if (mes == null) {
                        res = businessLogic.addContact(IName.getText(), IAddress.getText(), IPhone.getText());
                    } else {
                        res = businessLogic.updateContact(mes.get(0), IName.getText(), IAddress.getText(), IPhone.getText());
                    }
                    if(res){
